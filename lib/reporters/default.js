...
class Reporter {
	...
	writeTestSummary(event) {
		console.log(event, event.testFile, event.title);
		...
		} else if (event.type === 'test-timeout') {
			// TODO: title is not defined yet
			if (this.verbose) {
				this.write(`${colors.error(figures.circleDotted)} ${this.prefixTitle(event.testFile, event.title)}`);
			} else {
				this.write(this.prefixTitle(event.testFile, event.title));
			}
